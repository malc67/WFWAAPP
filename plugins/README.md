...
Now we introduce the plugins system to be able to add, remove, print, normalize every file/directory into the windowfilms during the installation.
...
| plugin.js    | `module.exports = { async apply(value, previousValues) { // Add code here }}` | The `apply` function is the one that apply the refactoring of the windowfilms. `value` is the result of the prompt, `previousValues` is previous responses of previous prompts and it's defined like `{ <plugin_name>: <value> }`. |
...
module.exports = {
  async apply(value, previousValues) {__
    return new Promise(resolve => {
      if (value) {
        console.log('TheCodingMachine React-Native windowFilms initialized the TYPESCRIPT please wait !',)
      } else {
        console.log('TheCodingMachine React-Native windowFilms initialized the JAVASCRIPT please wait !',)
      }
      resolve()
    })
  },
